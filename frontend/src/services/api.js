...
api.interceptors.response.use(
    ...
    (error) => {
        if (error.response?.status === 401) {
            ...
            // Using window.location to ensure a full page reload and context reset
...
export const patientDashboardAPI = {
    ...
    getBills: (patientId) => api.get('/bills/', { params: { patient: patientId } }),
    downloadBill: (id) => api.get(`/bills/${id}/download/`, { responseType: 'blob' }),
